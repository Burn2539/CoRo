...
Characteristic::~Characteristic()
{
	...
	cout << "Characteristic has been deleted." << endl;
}
...
HRESULT Characteristic::subscribeToNotification(__in bool _SubscritbeToNotification, __in bool _SubscribeToIndication ,__in PFNBLUETOOTH_GATT_EVENT_CALLBACK _callbackFunction)
{
	...
	if (FAILED(hr)) {
		cout << "\tERROR while enabling the notifications on characteristic " << CCCDescriptor->CharacteristicHandle << ": ";
		ErrorDescription(hr);
		return hr;
	}
...
HRESULT Characteristic::unsubscribeToNotification(void)
{
	...
	if (FAILED(hr)) {
		cout << "\tERROR while disabling the notifications on characteristic " << CCCDescriptor->CharacteristicHandle << ": ";
		ErrorDescription(hr);
		return hr;
	}
...
HRESULT Characteristic::writeValue(PBTH_LE_GATT_CHARACTERISTIC_VALUE _value)
{
	...
	if (FAILED(hr)) {
		cout << "ERROR while setting the characteristic's value: ";
		ErrorDescription(hr);
		return hr;
	}
